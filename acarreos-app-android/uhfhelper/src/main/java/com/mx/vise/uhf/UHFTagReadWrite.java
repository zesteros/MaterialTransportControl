...
public class UHFTagReadWrite {
    ...
    public boolean secureTag() {
        return writeAP(new Key(), new Key(SUCCESS_CODE), true);
    }

    private boolean writeAP(Key newKey, Key oldKey, boolean lock) {
        /*escribe contraseña*/
        /*
        ...
        while (tryToWriteAP) {
            writeAccessKey(newKey);
            this.tag.setAP(newKey);
            ap = readAP();
            tryToWriteAP = ap != null ? !ap.equals(newKey.key) : true;
            this.tag.setAP(oldKey);
    ...
    /**
     * @return if the tag has the AP Corrupted
     * test first the default ap, then test the new ap
     * for 5 seconds, if both give 09 code then the ap
     * is corrupted and the tag is damaged
     */
    public boolean isCorruptAP() {
        /*
         * Establish the default ap*/
        this.tag.setAP(new Key(SUCCESS_CODE));
        /*set start time*/
        long startTime = System.currentTimeMillis();
        /*while the ap be different than the default continue reading
         *
         * assume is corrupted until some password works*/
        boolean isCorrupted = true;
        /*
         * while  time elapsed be less than 3 seconds
         * */
        while (System.currentTimeMillis() - startTime < 800) {

            try {
                isCorrupted = !readAP().equals(new Key(SUCCESS_CODE).key);
                if (!isCorrupted)
                    return isCorrupted;
            } catch (Exception e) {
                e.printStackTrace();
        ...
        while (System.currentTimeMillis() - startTime < 800) {
            /*keep is corrupted false while the ap be different than the default ap*/
            try {
                isCorrupted = !readAP().equals(new Key().key);
                if (!isCorrupted)
                    return isCorrupted;
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public boolean unsecureTag() {
        return writeAP(new Key(SUCCESS_CODE), new Key(), false);
    }
