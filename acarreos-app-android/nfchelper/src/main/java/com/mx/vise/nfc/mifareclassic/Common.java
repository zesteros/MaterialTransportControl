...
import com.mx.vise.nfc.TagRead;
...
import static com.mx.vise.nfc.mifareclassic.Preferences.Preference.AutoCopyUID;
import static com.mx.vise.nfc.mifareclassic.Preferences.Preference.UIDFormat;
...
//    }
     ...
     * //
     */
    public static TagRead treatAsNewTag(Intent intent, Context context) {
        // Check if Intent has a NFC Tag.
        // if (NfcAdapter.ACTION_TECH_DISCOVERED.equals(intent.getAction())) {
        Tag tag = intent.getParcelableExtra(NfcAdapter.EXTRA_TAG);
        tag = MCReader.patchTag(tag);
        setTag(tag);
        int compatibility = checkMifareClassicSupport(tag, context);

        MifareClassicCompatibilityStatus mfcStatus = MifareClassicCompatibilityStatus.NO_COMPATIBLE;
        switch (compatibility) {
            case 0:
                mfcStatus = MifareClassicCompatibilityStatus.MIFARE_CLASSIC_FULL_COMPATIBLE;
                break;
            case -1:
                mfcStatus = MifareClassicCompatibilityStatus.NO_COMPATIBLE_HARDWARE;
                break;
            case -2:
                mfcStatus = MifareClassicCompatibilityStatus.NO_COMPATIBLE_TAG;
                break;
...
////        }
        return new TagRead(tag,mfcStatus);
        //}
        //return -4;
    }
