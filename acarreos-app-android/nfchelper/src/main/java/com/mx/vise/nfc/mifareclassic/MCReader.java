...
import static com.mx.vise.nfc.mifareclassic.MifareClassicReadWrite.DEFAULT_KEY;
import static com.mx.vise.nfc.mifareclassic.MifareClassicReadWrite.SIZE_OF_TAG_IN_SECTORS;
...
public class MCReader {
     ...
     */
    public SparseArray<String[]>  readAsMuchAsPossible(List<KeyPOJO> keys,boolean readVirgin) {
        SparseArray<String[]> resultSparseArray;
        if(keys == null && readVirgin){
            keys = new ArrayList<>();
            for(int i = 0; i < SIZE_OF_TAG_IN_SECTORS; i++){
                KeyPOJO keyPOJO = new KeyPOJO();
                keyPOJO.setKeyA(DEFAULT_KEY);
                keyPOJO.setKeyB(DEFAULT_KEY);
                keyPOJO.setSector(i);
                keys.add(keyPOJO);
            }
        }
        //if (keyMap != null && keyMap.size() > 0) {
        resultSparseArray = new SparseArray<>(16);
        ...
        for (KeyPOJO key : keys) {
            ...
            try {
                /*
                 * intenta leer el sector con las contraseñas dadas
                 * */
                results[0] = readSector(key.getSector(), Common.hexStringToByteArray(key.getKeyA()), false);
                results[1] = readSector(key.getSector(), Common.hexStringToByteArray(key.getKeyB()), true);

            } catch (TagLostException e) {
                /*
                 * Si se pierde el tag manda el nulo como indicador
                 * */
                return null;
            }
            // Merge results.
            if (results[0] != null || results[1] != null) {
                //Log.i(TAG, "readAsMuchAsPossible: se pudo leer, sector:"+key.getSector()+",results[0]="+results[0]+",results[1]="+results[1]+",key:"+key.getKeyA());
                /*
                 * Si se leyo correctamente pero es la contraseña default
                 * entonces termina, no hace falta leer los demas sectores
                 * se determina que el tag es virgen
                 * */
                if (key.getKeyA().equals(DEFAULT_KEY) && !readVirgin) {
                    /*
                     * Espera a que se la lista de sectores
                     * leidos sea mayor a 0 para evitar confundir
                     * al algoritmo de que ninguna contraseña fue
                     * correcta si no que si fue correcta solo
                     * la de defecto (fffff..)
                     * */
                    if (resultSparseArray.size() > 0) {
                        return resultSparseArray;
                    }
                }
                resultSparseArray.put(key.getSector(), mergeSectorData(results[0], results[1]));
            } else {
                //Log.i(TAG, "readAsMuchAsPossible: no se pudo leer, sector:"+key.getSector()+",results[0]="+results[0]+",results[1]="+results[1]+",key:"+key.getKeyA());
                /*
                 * Si se fallo alguna lectura no vale la pena seguir continuando
                 * se necesitan los 16 sectores completos
                 * */
                return resultSparseArray;
            }
    ...
    /**
     * @return Metodo para probar si es virgen (o si tiene la contraseña de defecto)
     */
    public boolean isVirgin(KeyPOJO key) {
        String[][] results = new String[2][];
        try {
            results[0] = readSector(key.getSector(), Common.hexStringToByteArray(DEFAULT_KEY), false);

            results[1] = readSector(key.getSector(), Common.hexStringToByteArray(DEFAULT_KEY), true);
        } catch (TagLostException e) {
            e.printStackTrace();
        }
...
//                                    Toast.LENGTH_LONG).show();
    ...
    private boolean authenticate(int sectorIndex, byte[] key,
        ...
        boolean retryAuth = false;//Common.getPreferences().getBoolean(
        //Preferences.Preference.UseRetryAuthentication.toString(), false);
        int retryCount = 1;// Common.getPreferences().getInt(
        //Preferences.Preference.RetryAuthenticationCount.toString(), 1);
        boolean ret = false;
